...
function renderSched(){
  ...
  emps.forEach(function(e,i){
    ...
    for(var d=1;d<=dim;d++){
      ...
      if(st.type==='none'&&st.base){cls='cell s-gone';txt='11';}
      ...
      if(st.type==='none')title=(st.base?'Не числится в этот день — смену нужно перекрыть':'Не числится в этот день')+' · '+empPeriodText(e);
    ...
    rows+='<tr class="'+(e.vacancy?'vacancy':'')+'">'+
      ...
      '<td class="c-fio sticky2">'+esc(empName(e))+
        (e.hiredAt||e.firedAt?' <span class="dt'+(e.firedAt?' out':'')+'">'+esc((e.hiredAt?'с '+fmtDate(e.hiredAt):'')+(e.hiredAt&&e.firedAt?' ':'')+(e.firedAt?'по '+fmtDate(e.firedAt):''))+'</span>':'')+
        '<span class="pos">'+esc(e.position||'')+(cover?' · перекрыть: '+cover:'')+'</span></td>'+
