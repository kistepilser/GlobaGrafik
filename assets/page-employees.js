...
function renderEmps(){
  ...
  var cards=objs.map(function(o){
    ...
    var rows=shown.length?shown.map(function(e,i){
      return '<tr'+(e.firedAt?' class="fired"':'')+'><td class="muted">'+(i+1)+'</td>'+
        ...
        '<td class="dates">'+(empPeriodTags(e)||'<span class="muted">бессрочно</span>')+'</td>'+
        '<td class="right">'+(isAdmin()
          ?'<button class="btn ghost sm" onclick="openEmpModal(&#39;'+e.id+'&#39;)">Изменить</button> '+
           '<button class="btn sm '+(e.firedAt?'ok':'warn')+'" onclick="openFireModal(&#39;'+e.id+'&#39;)">'+(e.firedAt?'Увольнение':'Увольнение')+'</button> '+
           '<button class="btn sm danger" onclick="delEmp(&#39;'+e.id+'&#39;)">Удалить</button>':'')+'</td></tr>';
    }).join(''):'<tr><td colspan="6" class="empty">'+(q?'Никто не найден по запросу':'В отделении пока нет сотрудников')+'</td></tr>';
    ...
    return '<div class="acc'+(open?' open':'')+'">'+
      ...
      '<div class="acc-body">'+
        ...
        '<th style="width:160px">Телефон</th><th style="width:95px">График</th>'+
        '<th style="width:250px">Приём / увольнение</th><th style="width:300px"></th></tr></thead>'+
  ...
  var toolbar='<div class="toolbar">'+
    ...
    '<div class="row"><button class="btn sm" onclick="openAllObjs(true)">Раскрыть всё</button>'+
      ...
      (isAdmin()?'<button class="btn warn sm" onclick="openFireModal()">Увольнение</button>'+
        '<button class="btn primary sm" onclick="openEmpModal()">＋ Сотрудник</button>':'')+'</div>'+
  '</div>';
...
function openEmpModal(id,objId){
  ...
  modal(e?'Изменить сотрудника':'Новый сотрудник',
    ...
    '<div class="grid2"><div><label class="f">Должность</label>'+posSelectHtml('e-pos',e?e.position:'')+'</div>'+
    ...
    '<div class="grid2"><div><label class="f">Работает с (первый день)</label>'+
      '<input id="e-from" type="date" value="'+esc(e&&e.hiredAt?e.hiredAt:'')+'"></div>'+
      '<div><label class="f">Последняя смена (увольнение)</label>'+
      '<input id="e-to" type="date" value="'+esc(e&&e.firedAt?e.firedAt:'')+'"></div></div>'+
    '<p class="muted" style="font-size:12.5px">Даты можно оставить пустыми — тогда сотрудник работает бессрочно. До даты приёма и после последней смены его смены автоматически попадают в «нужно перекрыть». Как только сотрудник занимает ставку, вакансия на этот график исчезает.</p>',
...
function saveEmp(id){
  var v={fio:document.getElementById('e-fio').value.trim(),phone:document.getElementById('e-phone').value.trim(),
    ...
    position:posValue('e-pos'),objectId:document.getElementById('e-obj').value,
    hiredAt:document.getElementById('e-from').value||'',
    firedAt:document.getElementById('e-to').value||''};
  if(v.hiredAt&&v.firedAt&&v.firedAt<v.hiredAt){toast('Дата увольнения раньше даты приёма');return;}
...
/* ---------- увольнение ---------- */
function openFireModal(id){
  if(!needAdmin())return;
  var staff=db.employees.filter(function(e){return !e.vacancy&&e.fio;});
  if(!staff.length){toast('Сначала добавьте сотрудников');return;}
  var cur=id?empById(id):staff[0];
  var today=new Date(),iso=today.getFullYear()+'-'+String(today.getMonth()+1).padStart(2,'0')+'-'+String(today.getDate()).padStart(2,'0');
  modal('Увольнение сотрудника',
    '<div class="field"><label class="f">Сотрудник</label><select id="f-emp" onchange="syncFireDate()">'+
      staff.map(function(e){
        var home=empHome(e);
        return '<option value="'+e.id+'"'+(cur&&e.id===cur.id?' selected':'')+'>'+esc(e.fio)+' — '+esc(home.code)+
          (e.position?' · '+esc(e.position):'')+(e.firedAt?' · уже уволен '+esc(fmtDate(e.firedAt)):'')+'</option>';
      }).join('')+'</select></div>'+
    '<div class="field"><label class="f">Последняя смена (включительно)</label>'+
      '<input id="f-date" type="date" value="'+esc(cur&&cur.firedAt?cur.firedAt:iso)+'"></div>'+
    '<p class="muted" style="font-size:12.5px">Сотрудник останется в графике до этого числа включительно — можно увольнять хоть серединой месяца. Все его смены после этой даты подсветятся как незакрытые, а его ставка со следующего месяца станет вакансией.</p>',
    '<button class="btn" onclick="cancelFire()">Отменить увольнение</button>'+
    '<button class="btn primary" onclick="applyFire()">Сохранить</button>');
}
function syncFireDate(){
  var e=empById(document.getElementById('f-emp').value);
  var i=document.getElementById('f-date');
  if(e&&i&&e.firedAt)i.value=e.firedAt;
}
function applyFire(){
  var e=empById(document.getElementById('f-emp').value);if(!e)return;
  var d=document.getElementById('f-date').value;
  if(!d){toast('Укажите дату последней смены');return;}
  if(e.hiredAt&&d<e.hiredAt){toast('Дата раньше даты приёма');return;}
  e.firedAt=d;
  save();closeModal();render();toast(e.fio+' — последняя смена '+fmtDate(d));
}
function cancelFire(){
  var e=empById(document.getElementById('f-emp').value);if(!e)return;
  delete e.firedAt;
  save();closeModal();render();toast(e.fio+' — увольнение отменено');
}
...
function openExtModal(id){
  ...
  modal(e?'Сотрудник из другого дивизиона':'Новый сотрудник из другого дивизиона',
    ...
    '<div class="grid2"><div><label class="f">Телефон</label><input id="x-phone" value="'+esc(e?e.phone:'')+'"></div>'+
      '<div><label class="f">Должность</label>'+posSelectHtml('x-pos',e?e.position:'')+'</div></div>'+
