...
var STATES={
  ...
  none:{label:'Ещё не работает / уже уволен',txt:'·',cls:'s-none'}
};

/* ---------- Должности ---------- */
var POSITIONS=['Управляющий','Товаровед'];
function posSelectHtml(id,cur){
  cur=cur||'';
  var known=(!cur)||POSITIONS.indexOf(cur)>=0;
  var v=cur||POSITIONS[0];
  return '<select id="'+id+'" onchange="togglePosOther(&#39;'+id+'&#39;)">'+
    POSITIONS.map(function(x){return '<option value="'+esc(x)+'"'+((known&&v===x)?' selected':'')+'>'+esc(x)+'</option>';}).join('')+
    '<option value="__other"'+(known?'':' selected')+'>Другая должность…</option></select>'+
    '<input id="'+id+'-other" class="'+(known?'hidden':'')+'" style="margin-top:8px" placeholder="Впишите должность" value="'+(known?'':esc(cur))+'">';
}
function togglePosOther(id){
  var s=document.getElementById(id),i=document.getElementById(id+'-other');
  if(!s||!i)return;
  var other=(s.value==='__other');
  i.classList.toggle('hidden',!other);
  if(other)i.focus();
}
function posValue(id){
  var s=document.getElementById(id);if(!s)return '';
  if(s.value!=='__other')return s.value;
  var i=document.getElementById(id+'-other');
  return i?i.value.trim():'';
}

/* ---------- Приём и увольнение ---------- */
function dnum(y,m,d){return y*10000+m*100+d;}
function parseDate(s){var t=/^(\d{4})-(\d{2})-(\d{2})$/.exec(String(s||''));return t?dnum(+t[1],+t[2],+t[3]):null;}
function fmtDate(s){var t=/^(\d{4})-(\d{2})-(\d{2})$/.exec(String(s||''));return t?((+t[3])+' '+MON_SHORT[(+t[2])-1].toLowerCase()+' '+t[1]):'';}
/* числится ли сотрудник в этот день */
function employedOn(e,y,m,d){
  if(!e)return false;
  var n=dnum(y,m,d),a=parseDate(e.hiredAt),b=parseDate(e.firedAt);
  if(a&&n<a)return false;
  if(b&&n>b)return false;
  return true;
}
/* работает ли хотя бы один день в этом месяце */
function employedInMonth(e,y,m){
  if(!e)return false;
  var a=parseDate(e.hiredAt),b=parseDate(e.firedAt);
  if(a&&a>dnum(y,m,daysInMonth(y,m)))return false;
  if(b&&b<dnum(y,m,1))return false;
  return true;
}
function empPeriodTags(e){
  var out='';
  if(e&&e.hiredAt)out+='<span class="tag">с '+esc(fmtDate(e.hiredAt))+'</span> ';
  if(e&&e.firedAt)out+='<span class="tag red">посл. смена '+esc(fmtDate(e.firedAt))+'</span> ';
  return out;
}
function empPeriodText(e){
  var a=e&&e.hiredAt?'с '+fmtDate(e.hiredAt):'',b=e&&e.firedAt?'по '+fmtDate(e.firedAt):'';
  return (a+(a&&b?' ':'')+b)||'без ограничений по датам';
}
...
function empById(id){
  ...
  if(String(id).indexOf('va:')===0){
    ...
    var pool=autoVacancies(parts[1],ui&&ui.year,ui&&ui.month).concat(autoVacancies(parts[1]));
    return pool.find(function(x){return x.id===id;});
  }
  return undefined;
}
...
function autoVacancies(objId,y,m){
  var o=objById(objId);if(!o)return [];
  var slots=slotsOf(o).slice();
  if(!slots.length)return [];
  db.employees.filter(function(e){return e.objectId===objId&&!e.external&&!e.vacancy&&(!y||employedInMonth(e,y,m));}).forEach(function(e){
...
function empsFor(objId,y,m){
  var own=db.employees.filter(function(e){return e.objectId===objId&&!e.external&&(!y||e.vacancy||employedInMonth(e,y,m));});
  return own.concat(autoVacancies(objId,y,m));
}
...
function cellState(emp,y,m,d){
  ...
  if(!emp.vacancy&&!employedOn(emp,y,m,d))return {type:'none',base:base,out:true};
  if(mk){
...
function needsCover(emp,st){
  if(st.type==='none')return st.base;
...
function availability(emp,y,m,d){
  var mk=db.marks[key(emp.id,y,m,d)];
  var base=isShift(emp.graph,y,m,d);
  if(!employedOn(emp,y,m,d)){
    var fired=parseDate(emp.firedAt)&&dnum(y,m,d)>parseDate(emp.firedAt);
    return {free:false,why:fired?('уволен '+fmtDate(emp.firedAt)):('принят с '+fmtDate(emp.hiredAt))};
  }
  if(mk){
...
function modal(title,body,foot){
  ...
  if(window.enhanceSelects)enhanceSelects(document.getElementById('modal-root'));
}
